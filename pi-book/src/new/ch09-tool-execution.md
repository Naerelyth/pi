...
```typescript
// packages/agent/src/agent-loop.ts:602-666（简化）
...
```typescript
// packages/agent/src/agent-loop.ts:668-709（简化）
...
```typescript
// packages/agent/src/agent-loop.ts:711-754（简化）
...
sequenceDiagram
    ...
    Note over Engine: === Sequential 模式 ===
    Engine->>P: prepare(tool_1)
    P-->>E: prepared
    E-->>F: executed
    F-->>Engine: result_1
    Engine->>P: prepare(tool_2)
    P-->>E: prepared
    E-->>F: executed
    F-->>Engine: result_2

    Note over Engine: === Parallel 模式 ===
    Engine->>P: prepare(tool_1)
    P-->>Engine: prepared_1
    Engine->>P: prepare(tool_2)
    P-->>Engine: prepared_2
    Engine->>E: execute(prepared_1) 同时开始
    Engine->>E: execute(prepared_2) 同时开始
    E-->>F: executed_1 (立即进入)
    F-->>Engine: finalized_1
    E-->>F: executed_2 (立即进入)
    F-->>Engine: finalized_2
    Engine->>Engine: 按源顺序生成结果
```
...
2. **Execute 和 Finalize 阶段并行**。所有通过 prepare 的工具调用会被封装成独立的异步任务并同时开始执行。当一个工具执行完成后，会**立即**进入 `finalize` 阶段（包括执行 `afterToolCall` 钩子），不再等待其他任务。
3. **消息生成按源顺序**。结果消息按 LLM 返回的工具调用顺序（不是执行完成顺序）生成和发射。

来看代码中 parallel 模式的关键段落：

```typescript
// packages/agent/src/agent-loop.ts:491-556（简化）
...
async function executeToolCallsParallel(...) {
  ...
  for (const toolCall of toolCalls) {
    ...
    if (preparation.kind === "immediate") {
      ...
      ...
      finalizedCalls.push(finalized); // 直接压入确定性结果
      ...
      continue;
    }

    // 将 execute 和 finalize 封装为异步任务闭包
...
注意 `finalizedCalls.push(async () => { ... })` 这个巧妙的设计。它把每个工具的 `execute` 和 `finalize` 绑定成一个闭包，然后在 `Promise.all` 中并发调用。这意味着只要一个工具执行完成，它就会立即进行自己的 `finalize` 阶段，不需要等待其他慢工具。

**为什么最终的消息生成要按源顺序？**

虽然 `afterToolCall` 钩子现在是并行执行的，但最终的结果必须要按照 LLM 原本输出工具的顺序组装成消息 `ToolResultMessage`。这不仅确保了 UI 呈现的一致性，更重要的是保证了对话历史 `context` 的**确定性**。无论每次并行执行完成的先后顺序如何，下一轮 LLM 看到的上下文（前一轮的工具返回结果序列）永远是确定不变的。
...
**Execute & Finalize 阶段**（并行）：
5. `read` 和 `edit` 封装成的异步闭包同时在 `Promise.all` 中启动
6. `read` 先完成执行（只是读磁盘），立即进入其自己的 `finalize` 阶段 — `afterToolCall` 可以脱敏文件内容，随后 emit `tool_execution_end`
7. `edit` 后完成执行（要写磁盘），立即进入其自己的 `finalize` 阶段 — `afterToolCall` 可以记录审计日志，随后 emit `tool_execution_end`

**消息生成阶段**（按源顺序）：
8. 等到两个异步闭包全部完成后，`Promise.all` 返回按源顺序排列的 `orderedFinalizedCalls` 数组
9. 先生成 `read` 的 `ToolResultMessage` 并 emit `message_start/end`
10. 再生成 `edit` 的 `ToolResultMessage` 并 emit `message_start/end`，最终一并返回。

注意一个微妙之处：如果两个工具调用中，`read` 的 prepare 失败了（比如 `beforeToolCall` 阻止了它），它的错误结果会在 prepare 循环中**立即**被加入 `finalizedCalls` 数组（作为一个已完成的对象而非闭包函数）。然后 `edit` 继续 prepare 并被推入闭包。最终 `Promise.all` 时，`read` 会直接被 `Promise.resolve` 处理，而 `edit` 会进行完整的并发执行。最终返回消息的顺序依然是：`read` 的错误结果 → `edit` 的成功结果。
...
> 本章核心分析基于 pi-mono v0.80.3（曾在 v0.66.0 中，`finalize` 为源顺序执行；近期版本已优化为 `execute` 和 `finalize` 闭包级并行处理，进一步提升了吞吐量）。
