...
pi-ai 的 OAuth 模块位于 `packages/ai/src/utils/oauth/`，支持 3 种 provider：
...
每种 provider 的 OAuth 流程差异巨大：Anthropic 用标准 PKCE 流程，GitHub Copilot 用 device code 流程，OpenAI Codex 用 PKCE 流程和 device code 流程。但对外暴露的接口是统一的。
...
**`onDeviceCode`** 是"请用户在设备上输入授权码"。参数包含 `userCode` 和 `verificationUri` 等。用于 GitHub Copilot 和 OpenAI Codex 的 device code 流程，终端实现会打印提示让用户在浏览器中输入对应的验证码。
...
**`onSelect`** 是"请用户选择一种登录方式"。当一个 Provider 提供多种登录途径时（比如 OpenAI Codex 支持浏览器登录和 device code 登录），通过这个回调让用户选择。

这个设计的关键洞察是：**OAuth 交互的本质是几个固定步骤（展示 URL、展示设备码、获取输入、报告进度、选择途径），但展示方式因环境而异。** 把步骤和展示分离，一套 OAuth 逻辑就能在所有环境中复用。
...
```typescript
// packages/ai/src/utils/oauth/anthropic.ts:238-239
...
> 本章核心分析基于 pi-mono v0.80.3。OAuth 模块是 pi-ai 中变化最频繁的部分——每次有新 provider 加入或现有 provider 更改认证方式，都需要更新。
> 近期变更移除了 Google 系列提供商（Gemini CLI 和 Antigravity），将内置提供商缩减为 3 种，并为 OpenAI Codex 增加了基于 `onSelect` 和 `onDeviceCode` 的设备码登录能力，使接口抽象更加完备。
> `OAuthProviderInterface` 接口自引入以来保持稳定，只增加了 `usesCallbackServer` 和 `modifyModels` 两个可选字段。
